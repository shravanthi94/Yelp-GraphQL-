...
import { signupCustomerMutation } from '../../mutations/mutations';
import '../../CSS/form.css';
import jwt_decode from 'jwt-decode';

const Signup = ({ signupCustomerMutation }) => {
  ...
  const onSubmit = async (e) => {
    e.preventDefault();
    console.log('here 1');
    const mutationResponse = await signupCustomerMutation({
      variables: {
        name: name,
        email: email,
        password: password,
    ...
    if (mutationResponse) {
      const response = mutationResponse.data.addCustomer;
      if (response) {
...
export default graphql(signupCustomerMutation, {
  name: 'signupCustomerMutation',
})(Signup);
